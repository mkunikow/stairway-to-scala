...
class Flight03 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  class ComplexNum(val real: Double, val imaginary: Double = 0) {
    def + (that: ComplexNum): ComplexNum = new ComplexNum(real + that.real, imaginary + that.imaginary )
    override def toString = s"$real ${if (imaginary < 0) "-" else "+"} ${math.abs(imaginary)}i"
  }

  implicit def DoubleToComlexNum(value: Double): ComplexNum = new ComplexNum(value)
