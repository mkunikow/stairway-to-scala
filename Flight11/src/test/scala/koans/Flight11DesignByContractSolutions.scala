...
class Flight11DesignByContractSolutions extends FunSuite with Matchers with SeveredStackTraces {

  test("Scala asserts and requires") {

    // bad color should be caught by the require
    intercept[IllegalArgumentException] {
      val cm1 = new ColorPairSolution("ELEPHANT")
    }

    // good color should be OK
    val cm2 = new ColorPairSolution("Red")
