...
class Flight02 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Map a tuple to strings") {
    ...
    val l = for (e <- t.productIterator.toList) yield e.toString
  ...
  test ("Immutable set in var") {
    ...
    // compilation error, collection immutable => getSet += => getSet = getSet +
  }
...
//    mutMap += (2 -> 2) // compilation error can't put Int to String
    mutMap(2) should be ("Two")
  }
