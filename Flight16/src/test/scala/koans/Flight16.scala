...
import koans.ObtuseJavaLib
import scala.collection.JavaConverters._


class Flight16 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Apply closures directly using the doMathFuncOnList method") {
    ...
    def applyFunc(s: Seq[Int], fn: (Int) => (Int)): List[Int] = {
      val jl: List[Integer] = s.toList.map( new java.lang.Integer(_) )

      obtuse.doMathFuncOnList(jl, new ObtuseJavaLib.MathFunc {
