...
class Flight04Solutions extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  def listShakespeareFiles(dirPath: String) : Array[String] = {
    ...
    val files = for (file <- fileList if file.getName.endsWith(".shkspr")) yield file.getName
    files.sorted
  }
  ...
  test ("shakespeare files contain question or statement?") {
    ...
    val shksprMap = Map.empty ++ {
      for {
        file <- listShakespeareFiles(".")
        which = statementOrQuestion(firstLineOfFile(file))
      } yield (file, which)   // adding tuples to a map adds a map entry - adding to empty map builds it up
    }
