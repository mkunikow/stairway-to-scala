...
class Flight04 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Refactoring var to val") {
    ...
    def max(numbers: List[Int]) = numbers match {
      case Nil => Int.MinValue
      case l => l.reduceLeft((x, y) => if (x > y) x else y)
    }
  ...
  def listShakespeareFiles(dirPath: String) : Array[String] = {
    ...
    for (file <- fileList; if file.isFile && file.getName.endsWith(".shkspr")) yield file.getName
  }
  // question: can you guess why we define this method outside of the test below?
  // answer: to use it in the last test?
  ...
  def statementOrQuestion(str : String) : String = str.trim.last match {
    case '?' => "Question"
    case _ => "Statement"
  }
  ...
  test ("shakespeare files contain question or statement?") {
    ...
    val shakespeareFilenames = listShakespeareFiles(".").toList
    val list = for {
      name <- shakespeareFilenames
      content = statementOrQuestion(firstLineOfFile(name))
    } yield (name, content)

    val shksprMap = list.toMap
