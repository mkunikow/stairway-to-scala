...
class Flight05 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test ("Filter numbers") {
    ...
    val onlyOdd = allNumbers.filter(_ % 2 != 0)
    val onlyEven = allNumbers.filter(_ % 2 == 0)
  ...
  test ("Multiply variable number of arguments") {
    ...
    def multiplyDoubles(x: Double*): Double = x.foldLeft(1.0)(_ * _)
...
//    def listOfLists(vargs: String*): List[List[String]] = {
//      if (vargs.size == 0) Nil
//      else vargs.toList :: listOfLists(vargs.tail:_*)
//    }

    //tailrec
    def listOfLists(vargs: String*): List[List[String]] = {
      @tailrec
      def listOfListsAcc(acc: List[List[String]],  vargs_inner: String*): List[List[String]] = {
        if (vargs_inner.size == 0) acc
        else listOfListsAcc(vargs_inner.toList :: acc, vargs_inner.tail:_*)
      }
      listOfListsAcc(Nil, vargs:_*).reverse
    }
  ...
  test ("URL cleaner") {
    def urlClean(urlAsString : String) : URL = {
      Try(new URL(urlAsString)) getOrElse new URL("http://badurl.com")
    }
